...
"""
Semantic Annotator
- Fuses YOLO detections with RTAB-Map grid.
- Publishes semantic grid + RViz markers + JSON layer feed.
...
class SemanticAnnotator:
    def __init__(self):
        ...
        # Tuning: distance threshold for duplicate detection
        self.duplicate_threshold = 10.0  # meters
        self.duplicate_count = 0

        self.marker_id = 0
        rospy.loginfo("Semantic Annotator ready")

    # -------------------------------------------------------------
    def rtabmap_callback(self, msg):
        """Store base RTAB-Map occupancy grid."""
        self.rtabmap_grid = np.array(msg.data, dtype=np.int8).reshape(
    ...
    def yolo_callback(self, msg):
        """Add YOLO detections into semantic layers."""
        ...
        try:
            ...
            try:
                ...
                rospy.logwarn_throttle(5, "TF lookup failed; skipping YOLO detections")
                return
            ...
            for det in detections:
                ...
                # Tuning: forward projection distance for approximated detection position
                est_range = 5.0  # meters
            ...
            # Semantic JSON for higher-level nodes
    ...
    def publish_semantic_map(self):
        """Fuse semantic overlays into occupancy grid."""
        ...
        for category, detections in self.semantic_layers.items():
            ...
            size = {'rocks': 3, 'base': 5, 'flags': 2, 'antennas': 2}[category]  # radius in cells
        ...
        total = sum(len(v) for v in self.semantic_layers.values())
        rospy.loginfo_throttle(10, f"Semantic map published | detections={total} | duplicates={self.duplicate_count}")

    # -------------------------------------------------------------
    def publish_semantic_markers(self):
        """Publish RViz markers for detection visualization."""
        ...
        rospy.loginfo_throttle(15, f"Semantic markers published: {len(marker_array.markers)//2}")
